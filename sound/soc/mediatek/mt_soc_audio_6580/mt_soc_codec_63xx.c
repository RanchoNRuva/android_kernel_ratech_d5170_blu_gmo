...
//+++++rgk lihua 2015-01-30  start
extern void Sound_Speaker_Turnon(int channel);
extern void Sound_Speaker_Turnoff(int channel);
//+++++rgk lihua 2015-01-30  end
...
#endif
//add by xuliu for new speaker mode 2016-3-7
#ifdef MTK_AUDIO_USING_SPEAKER_MODE_AB

static int Speaker_mode = AUDIO_SPEAKER_MODE_AB;	/* default use type D on 82 */
#else
...
//+++++rgk lihua 2015-01-30  start    
    if (enable)
    {
        Sound_Speaker_Turnon(1);
    }
    else
    {
        Sound_Speaker_Turnoff(0);
    }
//+++++rgk lihua 2015-01-30  end     
/*
...
static void mt6350_codec_init_reg(struct snd_soc_codec *codec)
{
	...
	Ana_Set_Reg(DRV_CON2, 0x08CC, 0xFFFF);
	pr_warn("%s, AUD_MISO driving modification\n", __func__);
