...
#include "msg22xx_hlt_v9_update_bin.h" // for MSG22xx   //helitai
#include "msg22xx_dj_v11_update_bin.h"   //junda
...
#endif //CONFIG_ENABLE_TYPE_B_PROTOCOL
s32 DrvFwCtrlEnableProximity_exp(void);
        ...
        if (bIsCorrectFormat) 
        {
            ...
            switch (nWakeupMode)
            {
                case 0x58:
                    ...
                    input_report_key(g_InputDevice, KEY_F, 1);
                    input_sync(g_InputDevice);
                    input_report_key(g_InputDevice, KEY_F, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x60:
...
#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
if( 1== g_EnableTpProximity )
DrvFwCtrlEnableProximity_exp();
#endif
    DBG("received raw data from touch panel as following:\n");
        ...
        if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF))
        {
            ...
            {   /* 0x00 is key up, 0xff is touch screen up */
                ...
                if (tpd_dts_data.use_tpd_button)
                {
                    ...
                    else if (pPacket[5] == 4) // TOUCH_KEY_MENU
                    {
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    u16 pMajor=0;
		...
		if(pMajor==0x01){
		len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","HELITAI",0x26,pMajor,pMinor,SAMPLE_FWVER_HELITAI_MAJOR,SAMPLE_FWVER_HELITAI_MINOR);
		}else{
		len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","DIJIN",0x26,pMajor,pMinor,SAMPLE_FWVER_DIJIN_MAJOR,SAMPLE_FWVER_DIJIN_MINOR );
		}
...
s32 DrvFwCtrlEnableProximity(void)
{
    ...
    if (rc > 0)
    {
        ...
        DBG("sym Enable proximity detection success\n");
    }
    else
    {
        DBG("sym Enable proximity detection failed\n");
    }
#endif
    mutex_unlock(&g_Mutex);
	return 1;
...
s32 DrvFwCtrlDisableProximity(void)
{
    ...
    if (rc > 0)
    {
        ...
        DBG("sym Disable proximity detection success\n");
    }
    else
    {
        DBG("sym Disable proximity detection failed\n");
    }
