...
#endif //CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
{
    ...
    ) 
    {
        ...
        _gInterruptFlag = 0;
DBG("111111111111111111111111111111111111111111111111\n"); 
        schedule_work(&_gFingerTouchWork);
    }
#else    
    if (_gInterruptFlag == 1
    ...
    {    
        ...
        _gTpdFlag = 1;
        DBG("222222222222222222222222222222222222222222222\n"); 
...
/*
    ...
    mdelay(25); 
*/
DBG("***MS_TS_MSG_IC_GPIO_RST is  %d  ***\n", MS_TS_MSG_IC_GPIO_RST);
DBG("***MS_TS_MSG_IC_GPIO_INT is  %d  ***\n", MS_TS_MSG_IC_GPIO_INT);
    ...
    tpd_gpio_output(MS_TS_MSG_IC_GPIO_INT, 1);
    mdelay(20);
...
void DrvPlatformLyrDisableFingerTouchReport(void)
{
 ...
 //   spin_lock_irqsave(&_gIrqLock, nIrqFlag);             //sym modified for sometimes reboot in 20161215
...
*/
    ...
    input_set_capability(g_InputDevice, EV_KEY, KEY_F);
           ...
           //pDeviceNode = of_find_matching_node(pDeviceNode, touch_of_match);
            ...
            if (pDeviceNode)
            {
              ...
              // nRetVal = request_irq(_gIrq/*MS_TS_MSG_IC_GPIO_INT*/, _DrvPlatformLyrFingerTouchInterruptHandler,
               ...
               //      "TOUCH_PANEL-eint", NULL); 
                DBG("*** wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwable to claim irq %d ***\n", _gIrq);
                if (nRetVal != 0)
                {
                    DBG("*** Unable to claim irq %d; error %d ***\n", _gIrq, nRetVal);
