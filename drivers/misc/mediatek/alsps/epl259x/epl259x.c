...
#define APS_FUNC() 	pr_err(APS_TAG"%s %d \n ", __func__, __LINE__)
...
static struct pinctrl_state *pins_cfg;
 ...
 //   struct hwm_sensor_data sensor_data;
	...
	APS_LOG("interrupt_flag is %d\n", epl_sensor.ps.interrupt_flag);
        if(epl_sensor.ps.interrupt_flag == EPL_INT_TRIGGER && !obj->hw->polling_mode_ps)
        {
         ...
         APS_LOG("ps report data  is %d\n", epl_sensor.ps.compare_low >> 3);   
          ps_report_interrupt_data(epl_sensor.ps.compare_low >> 3);
#if PS_DYN_K
...
/*----------------------------------for factory mode test---------------------------------------*/
		case ALSPS_GET_PS_TEST_RESULT:
				APS_LOG("ALSPS_GET_PS_TEST_RESULT\n");
            if(enable_ps == 0)
            {
                set_bit(CMC_BIT_PS, &obj->enable);
                epl_sensor_update_mode(obj->client);
		...
		case ALSPS_SET_PS_THRESHOLD:
				APS_LOG("ALSPS_SET_PS_THRESHOLD\n");
		...
		case ALSPS_GET_PS_THRESHOLD_HIGH:
				APS_LOG("ALSPS_GET_PS_THRESHOLD_HIGH\n");
#if 0
		...
		case ALSPS_GET_PS_THRESHOLD_LOW:
				APS_LOG("ALSPS_GET_PS_THRESHOLD_LOW\n");
#if 0
...
static int als_enable_nodata(int en)
{
	...
	APS_LOG("epl_sensor_obj als enable value = %d\n", en);

	mutex_lock(&sensor_mutex);
	if (en)
		set_bit(CMC_BIT_ALS, &epl_sensor_obj->enable);
	else
		clear_bit(CMC_BIT_ALS, &epl_sensor_obj->enable);
	mutex_unlock(&sensor_mutex);
	if (!epl_sensor_obj) {
		APS_ERR("epl_sensor_obj is null!!\n");
		return -1;
	}
	  epl_sensor_update_mode(epl_sensor_obj->client);
	return 0;
}
...
{
	int err = 0;
	struct epl_sensor_priv *obj = NULL;
	...
	obj = epl_sensor_obj;
	err = epl_sensor_read_als(obj->client);
	if (err) {
		err = -1;
	} else {
...
static int ps_enable_nodata(int en)
{
	...
	APS_LOG("epl_sensor_obj  ps enable value = %d\n", en);
	mutex_lock(&sensor_mutex);
	if (en)
		set_bit(CMC_BIT_PS, &epl_sensor_obj->enable);
	else
		clear_bit(CMC_BIT_PS, &epl_sensor_obj->enable);
	mutex_unlock(&sensor_mutex);
	if (!epl_sensor_obj) {
		APS_ERR("epl_sensor_obj is null!!\n");
		return -1;
...
//	pinctrl_select_state(pinctrl, pins_cfg);
	  epl_sensor_update_mode(epl_sensor_obj->client);
	     ...
	     epl_sensor.ps.compare_reset = EPL_CMP_RUN;
            epl_sensor.ps.lock = EPL_UN_LOCK;
            epl_sensor_I2C_Write(epl_sensor_obj->client, 0x1b, epl_sensor.ps.compare_reset |epl_sensor.ps.lock);
APS_FUNC();
eint_flag = true;
			enable_irq(epl_sensor_obj->irq);	
#endif			
...
APS_FUNC();
	...
	err = epl_sensor_read_ps(epl_sensor_obj->client);
	 mutex_lock(&sensor_mutex);
        epl_sensor_read_ps_status(epl_sensor_obj->client);
        mutex_unlock(&sensor_mutex);
	if (err) {
		err = -1;
	} else {
