...
//rgk wanghanfeng add for *#*#9375#*#* start (2015-11-30)
#include <linux/seq_file.h>
#include <linux/proc_fs.h> 
//rgk wanghanfeng add for *#*#9375#*#* end (2015-11-30)
...
static int rgk_lcm_info_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n", mtkfb_lcm_name);	
	return 0;
}

static int rgk_lcm_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, rgk_lcm_info_show, NULL);
}

static const struct file_operations rgk_lcm_info_proc_fops = {	
	.owner      = THIS_MODULE,
	.open       = rgk_lcm_info_open,
	.read       = seq_read,
	.llseek     = seq_lseek,
...
{
	struct proc_dir_entry *lcm_info_entry = proc_create("rgk_lcdInfo", 0444, NULL, &rgk_lcm_info_proc_fops);
	if (lcm_info_entry == NULL)
		pr_err("create /proc/lcm_info_entry fail\n");
