...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	pr_err("charger_vol = %d\n",charger_vol);
	pr_err("is_on = %d\n",is_on);
	if(is_on && (charger_vol > 4000))
	{
		switch_set_state((struct switch_dev *)&otg_state, 1);
		pr_err("otg_state_on\n");
	}
	else
	{
		switch_set_state((struct switch_dev *)&otg_state, 0);
		pr_err("otg_state_off\n");
	}
#endif
}
...
static bool musb_is_host(void)
{
	...
	DBG(0, "iddig_state = %d\n", iddig_state);
	DBG(0, "iddig_pin = %d\n", iddig_pin);
	DBG(0, "iddig_pin_mode = %d\n", iddig_pin_mode);
	DBG(0, "drvvbus_pin = %d\n", drvvbus_pin);
	DBG(0, "drvvbus_pin_mode = %d\n", drvvbus_pin_mode);
//	DBG(0, "GPIO_OTG_IDDIG_EINT_PIN = %d\n", GPIO_OTG_IDDIG_EINT_PIN);
#endif
#else
	iddig_state = 0;
...
static void otg_int_init(void)
{
	...
	ret = request_irq(usb_iddig_number, mt_usb_ext_iddig_int, IRQF_TRIGGER_LOW, "USB_IDDIG", NULL);
		...
		ret = request_irq(usb_iddig_number, mt_usb_ext_iddig_int, IRQF_TRIGGER_LOW, "OTG_IDDIG-eint", NULL);
	#endif
	if (ret > 0)
