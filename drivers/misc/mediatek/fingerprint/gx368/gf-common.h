...
#include <../../../misc/mediatek/include/mt-plat/mt6580/include/mach/gpio_const.h>
//#include <mach/mt_spi.h>
...
#define     GF_IRQ_NUM         1
#define     GF_IRQ_PIN_NUM     1
#define     GF_IRQ_PIN         (GPIO18 | 0x80000000)
#define     GF_IRQ_PIN_M_GPIO  GPIO_MODE_00
#define     GF_IRQ_PIN_M_EINT  GPIO_MODE_02
  
#define     GF_RST_PIN         (GPIO81 | 0x80000000)
#define     GF_RST_PIN_M_GPIO  GPIO_MODE_00
#define     GF_RST_PIN_M_DAIPCMOUT   GPIO_MODE_01
 
#define     GF_SCK_PIN         (GPIO1 | 0x80000000)            
#define     GF_SCK_PIN_M_GPIO  GPIO_MODE_00
#define     GF_SCK_PIN_M_SCK   GPIO_MODE_01 

#define     GF_CS_PIN          (GPIO0 | 0x80000000)
#define     GF_CS_PIN_M_GPIO   GPIO_MODE_00
#define     GF_CS_PIN_M_CS     GPIO_MODE_01
     
#define     GF_MOSI_PIN        (GPIO2 | 0x80000000)
#define     GF_MOSI_PIN_M_GPIO GPIO_MODE_00
#define     GF_MOSI_PIN_M_MOSI GPIO_MODE_01
...
inline static long spi_dts_gpio_init(gf_dev_t *pdev)
{
	long ret = 0;
	//struct pinctrl *pctrl;
	struct pinctrl_state *pState = 0;
	...
	/* retrieve */
	gf_debug(DEFAULT_DEBUG,"nasri...xielei spi_dts_gpio_init\n");
	
	//pctrl = devm_pinctrl_get(&pdev->dev);
	node = of_find_compatible_node(NULL, NULL,"mediatek,gx368");
	#if 1
	if(node){
		gf_debug(DEFAULT_DEBUG,"nasri...xielei spi_dts_gpio_init get node success\n");
		#if 1
		for(i=0;i<DTS_GPIO_STATE_MAX;i++){
			pState = pinctrl_lookup_state(pdev->pctrl, this_state_name[i]);
			if (IS_ERR(pState)) {
				printk("nasri...xielei spi_dts_gpio_init gpio_name=%s failed\n",  this_state_name[i]);
				ret = PTR_ERR(pState);
				return ret;
			}
		}
		#endif
		gf_debug(DEFAULT_DEBUG,"nasri...spi_dts_gpio_init success xielei\n");
		this_pctrl = pdev->pctrl;
	}else{
		printk("nasri...xielei spi_dts_gpio_init get node failed\n");
  }
	#endif
//exit:
	return ret;
