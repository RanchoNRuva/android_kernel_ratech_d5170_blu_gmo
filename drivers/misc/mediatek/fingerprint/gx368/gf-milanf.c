...
//static struct pinctrl_state *eint_as_int, *eint_pulldown, *eint_pulldisable, *rst_output0, *rst_output1, *finger_power_on, *finger_power_off;

struct of_device_id fingerprint_of_match[] = {
//	{ .compatible = "mediatek,mt6735-fingerprint", },
	{ .compatible = "mediatek,hct_finger", },
//	{ .compatible = "mediatek,mt6580-fingerprint", },
//	{ .compatible = "mediatek,mt8173-fingerprint", },
//	{ .compatible = "mediatek,mt6755-fingerprint", },
//	{ .compatible = "mediatek,mt6797-fingerprint", },
//	{ .compatible = "mediatek,mt8163-fingerprint", },
	{},
};
...
//{
//	/*cs*/
//    mt_set_gpio_mode(GF_CS_PIN, GF_CS_PIN_M_CS);
//
//	/*sck*/
//    mt_set_gpio_mode(GF_SCK_PIN, GF_SCK_PIN_M_SCK);
//
//	/*miso*/
//    mt_set_gpio_mode(GF_MISO_PIN, GF_MISO_PIN_M_MISO);
//
//	/*mosi*/
//    mt_set_gpio_mode(GF_MOSI_PIN, GF_MOSI_PIN_M_MOSI);
//
 //   msleep(1);
//}
...
inline static void gf_irq_cfg(gf_dev_t *gf_dev)
{
	...
	//mt_set_gpio_mode(gf_dev->irq_gpio, GF_IRQ_PIN_M_EINT);
	//mt_set_gpio_dir(gf_dev->irq_gpio, GPIO_DIR_IN); 
	//mt_set_gpio_pull_enable(gf_dev->irq_gpio, GPIO_PULL_DISABLE);
...
#if 0
	mt_set_gpio_mode(gf_dev->rst_gpio, GF_RST_PIN_M_GPIO);
	mt_set_gpio_dir(gf_dev->rst_gpio, GPIO_DIR_OUT);
	mt_set_gpio_out(gf_dev->rst_gpio, GPIO_OUT_ZERO); 			 
	msleep(10);  //delay for power to reset  typical:10ms max:50ms
	mt_set_gpio_out(gf_dev->rst_gpio, GPIO_OUT_ONE);
#else
...
int gf_power_on(gf_dev_t *gf_dev, bool onoff)
{
	...
	if(onoff) {
		if (0 == gf_dev->poweron){
			...
			ret = regulator_enable(g_RegVdd);	/*enable regulator*/
...
static int fingerprint_get_gpio_info(struct platform_device *pdev)
{
	...
	printk("[fingerprint %d] mt_fingerprint_pinctrl+++++++++++++++++\n", pdev->id);
	if(pdev == NULL)
	{
...
//  finger_power_on = pinctrl_lookup_state(pinctrl1, "finger_power_en1");
//  if (IS_ERR(finger_power_on)) {
//  	ret = PTR_ERR(finger_power_on);
//    dev_err(&pdev->dev, " Cannot find hct_finger pinctrl finger_power_on!\n");
//    return ret;
//  }   
//  finger_power_off = pinctrl_lookup_state(pinctrl1, "finger_power_en0");
//  if (IS_ERR(finger_power_off)) {
//    ret = PTR_ERR(finger_power_off);
//    dev_err(&pdev->dev, " Cannot find hct_finger pinctrl finger_power_off!\n");
//    return ret;
//  }

	eint_as_int = pinctrl_lookup_state(pinctrl1, "goodix_pin_irq");
	//printk("nasri....eint_as_int = %s\n",eint_as_int->name);
...
//	eint_pulldown = pinctrl_lookup_state(pinctrl1, "state_eint_pull_down");
...
//		return ret;
//	}
//	eint_pulldisable = pinctrl_lookup_state(pinctrl1, "state_eint_pull_disable");
...
//	}
	...
	rst_output0 = pinctrl_lookup_state(pinctrl1, "gx368_reset_low");
	//printk("nasri....rst_output0 = %s\n",rst_output0->name);
	if (IS_ERR(rst_output0)) {
		ret = PTR_ERR(rst_output0);
		dev_err(&pdev->dev, "fwq Cannot find nasri_fingerprint pinctrl state_rst_output0!\n");
		return ret;
	}
	rst_output1 = pinctrl_lookup_state(pinctrl1, "gx368_reset_high");
	//printk("nasri....rst_output1 = %s\n",rst_output1->name);
	if (IS_ERR(rst_output1)) {
		ret = PTR_ERR(rst_output1);
		dev_err(&pdev->dev, "fwq Cannot find nasri_fingerprint pinctrl state_rst_output1!\n");
		return ret;
	}
	//fingerprint_DEBUG("[fingerprint%d] mt_fingerprint_pinctrl----------\n", pdev->id);
	//pinctrl_select_state(pinctrl1, finger_power_on);
...
static int  gf_probe(struct spi_device *spi)
{
	...
	#if 1
	printk("nasri...gf_probe111111111111111\n");
	spi->dev.of_node=of_find_compatible_node(NULL, NULL, "mediatek,hct_finger");
	printk("nasri...gf_probe222222222222222222\n");
	gf_dev->pctrl = devm_pinctrl_get(&spi->dev);
	printk("nasri...gf_probe333333333333333333333\n");
...
#if 1
	printk("nasri...gf_probe4444444444444444444\n");
	spi_dts_gpio_init(gf_dev);
	printk("nasri...gf_probe55555555555555555555\n");
#endif
...
//	pinctrl_select_state(pinctrl1, eint_as_int);
	...
	/*SPI parameters.*/	
	gf_spi_setup(gf_dev, 8*1000*1000);// no function add by cary
	gf_spi_set_mode(gf_dev->spi, SPEED_1MHZ, 0);
	
	gf_hw_reset(gf_dev,60);

	printk("%s dev irq_pin is %d,rst_pin is %d. \n",__func__,gf_dev->irq_gpio ,gf_dev->rst_gpio);
	//nasri
	//gf_spi_pins_config();
	...
	node = of_find_compatible_node(NULL,NULL, "mediatek,gx368");
	...
	if (node) {
		...
		ret = request_irq(gf_dev->spi->irq, (irq_handler_t) fingerprint_eint_interrupt_handler, IRQF_TRIGGER_RISING,"FINGERPRINT-eint", NULL);
